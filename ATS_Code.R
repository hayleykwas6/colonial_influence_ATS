...
df$InBRICS            <- as.factor(df$InBRICS)
...
## remove variables with > 2 and re-check
...
predictor_names <- rownames(adonis_results[[1]])
keep_rows       <- !predictor_names %in% c("Residual", "Total")
predictor_names <- predictor_names[keep_rows]

# Pool R2, F, SD across imputations
pooled_r2 <- colMeans(r2_values, na.rm = TRUE)[keep_rows]
pooled_f  <- colMeans(f_values,  na.rm = TRUE)[keep_rows]
pooled_sd <- apply(r2_values, 2, sd, na.rm = TRUE)[keep_rows]

# Extract and filter p-value matrix
p_matrix  <- do.call(rbind, lapply(adonis_results, function(x) x$`Pr(>F)`))[, keep_rows]

# Median p-value (primary — avoids permutation floor issues)
median_p  <- apply(p_matrix, 2, median, na.rm = TRUE)

# Fisher's combined probability (-2 * sum(log(p)) ~ chi-sq with 2m df)
fisher_p  <- apply(p_matrix, 2, function(ps) {
  ps <- ps[!is.na(ps) & ps > 0]
...
results_table <- data.frame(
  ...
  Fisher_P      = ifelse(fisher_p < 0.0001, "< 0.0001", as.character(round(fisher_p, 4))),
  Significance  = ifelse(median_p < 0.001, "***",
                         ifelse(median_p < 0.01,  "**",
                                ifelse(median_p < 0.05,  "*", "ns")))
)
...
# binned and imputation approaches aligned with complete cases (na.omit) 
# complete cases is primary since all three approaches converge on the same significant predictors

## ===== Final Step: Remove Clearly Insignificant variables =====

adonis_complete_sig <- adonis2(dist_complete ~ Mil_Per + Colonization.Status + InNATO + InBRICS + Continent,
                             data         = df_complete,
                             by           = "margin", permutations = 9999)
print(adonis_complete_sig)
